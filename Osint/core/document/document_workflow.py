# core/document/document_workflow.py
...
class DocumentWorkflow:
    """Ekspor hasil scan dokumen."""

    def __init__(self, base_dir: str = "Osint"):
        self.base_dir = base_dir
        self.output_root = os.path.join(base_dir, "results", "document")

    def run(self, target: str, results: List[ScanResult]) -> str:
        safe_target = self._sanitize_name(os.path.basename(target))
        ...
        console.print(f"[green]Document summary exported to {user_dir}[/green]")
        return user_dir
    ...
    def _export_json(self, results: List[ScanResult], user_dir: str) -> None:
        ...
        path = os.path.join(user_dir, "document_report.json")
        with open(path, "w", encoding="utf-8") as f:
            json.dump(data, f, indent=4, ensure_ascii=False)

    def _export_csv(self, results: List[ScanResult], user_dir: str) -> None:
        path = os.path.join(user_dir, "document_details.csv")
    ...
    def _export_markdown(self, target: str, results: List[ScanResult], user_dir: str) -> None:
        path = os.path.join(user_dir, "document_report.md")
        md = f"# Document Intelligence Report\n\n**Target:** {target}\n\n"
        for r in results:
            md += f"## {r.platform}\n"
