# core/domain/domain_workflow.py
...
class DomainWorkflow:
    """Ekspor hasil scan domain ke JSON, CSV detail, dan Markdown (semua platform digabung)."""

    def __init__(self, base_dir: str = "Osint"):
        self.base_dir = base_dir
        self.output_root = os.path.join(base_dir, "results", "domain")
    ...
    def run(self, target: str, results: List[ScanResult]) -> str:
        ...
        console.print(f"[green]Domain summary exported to {user_dir}[/green]")
        return user_dir
    ...
    def _export_json(self, results: List[ScanResult], user_dir: str) -> None:
        ...
        path = os.path.join(user_dir, "domain_report.json")
        with open(path, "w", encoding="utf-8") as f:
            json.dump(data, f, indent=4, ensure_ascii=False)

    def _export_csv(self, results: List[ScanResult], user_dir: str) -> None:
        path = os.path.join(user_dir, "domain_details.csv")
    ...
    def _export_markdown(self, target: str, results: List[ScanResult], user_dir: str) -> None:
        path = os.path.join(user_dir, "domain_report.md")
        md = f"# Domain Intelligence Report\n\n**Target:** {target}\n\n"
        for r in results:
            md += f"## {r.platform}\n"
